...
class NaiveBayesClassifier(object):
    ...
    def fit(self, trainingData, trainingLabels):
        ...
        for i, j in zip(trainingData, trainingLabels):
            for f in self.features:
                self.conditionalProb[(f, j, i[f])] += 1

        if self.k != 0:
            for key, value in self.conditionalProb.iteritems():
                for i in self.featureValues:   
                    n = self.prior[key[1]]
                    self.conditionalProb[key] = (value + 1.0) / (n + len(self.featureValues[i]) + 1.0)
        else:
            for key, value in self.conditionalProb.iteritems():
                n = self.prior[key[1]]
                self.conditionalProb[key] = value / float(n)
    ...
    def calculateJointProbabilities(self, instance):
        ...
        for label in self.legalLabels:
            ...
            for cb in instance.keys():
                if (cb, label, instance[cb]) in self.conditionalProb.keys():
                    joint[label] *= self.conditionalProb[(cb, label, instance[cb])]
    ...
    def calculateLogJointProbabilities(self, instance):
        ...
        for label in self.legalLabels:
            ...
            for cb in instance.keys():
                #if (cb, label, instance[cb]) in self.conditionalProb.keys():
                logJoint[label] += math.log(self.conditionalProb[(cb, label, instance[cb])])
