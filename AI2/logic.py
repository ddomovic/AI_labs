...
def resolution(clauses, goal):
    ...
    while True:
        ...
        clause_1, clause_2 = selectClauses(clauses, setOfSupport, resolvedPairs)
        if clause_1 == "":
            return False
        aux_c = negate_all_literals(clause_1)
        if aux_c.literals == clause_2.literals:
            return True
        else:
            resolvedPairs.add((clause_1, clause_2))
            resolved_p = resolvePair(clause_1, clause_2)
            setOfSupport.update(resolved_p)
...
def removeRedundant(clauses, setOfSupport):
    ...
    for c in clauses:
        if not c.isRedundant(clauses):
            aux_clauses.add(c)
    for s in setOfSupport:
        if not s.isRedundant(setOfSupport):
            aux_sos.add(s)

    return aux_clauses, aux_sos
...
def resolvePair(firstClause, secondClause):
    ...
    return_clauses = set()
    for lit1 in firstClause.literals:
        if lit1.negate() in secondClause.literals:
            result = clause_without(secondClause, lit1.negate())
            if not len(result.literals) == 0:
                return_clauses.add(result)
    for lit2 in secondClause.literals:
        if lit2.negate() in firstClause.literals:
            result = clause_without(firstClause, lit2.negate())
            if not len(result.literals) == 0:
                return_clauses.add(result)
...
def selectClauses(clauses, setOfSupport, resolvedPairs):
    ...
    for c in clauses.union(setOfSupport):
        for s in setOfSupport:
            if (c, s) not in resolvedPairs and c.isResolveableWith(s) and (s, c) not in resolvedPairs and s.isResolveableWith(c):
                return c, s
            if c.isResolveableWith(c) and not len(c.literals) == 1:
                return c, c
            if s.isResolveableWith(s) and not len(s.literals) == 1:
                return s, s
